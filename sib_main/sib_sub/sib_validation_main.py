...
def calculate_distance(nearest_neighbour):
    ...
    nearest_neighbour['distance'] = nearest_neighbour.distance(nearest_point)  # calculate distance with ground_truth
...
def calculate_tp(nearest_neighbour):
    ...
    # Sorts by closest distance and takes this as the TP - This is the value that will get deleted from ground_truth and model_det.
    nearest_neighbour = nearest_neighbour.sort_values(by='distance', ascending=True).drop_duplicates(subset='n_ID', keep='first', inplace=False)
...
def calculate_fp(nearest_neighbour):
    ...
    # Sorts by closest distance and takes this as the FP - This is the value that will get deleted from ground_truth and model_det.
    nearest_neighbour = nearest_neighbour.sort_values(by='distance', ascending=True).drop_duplicates(subset='n_ID', keep='first', inplace=False)
    ...
    if len(FP1) > 0:
        ...
        FP1 = FP1[['gt_ID', 'n_det_class', 'n_ID', 'nearest_point', 'FP']]
        FP1 = FP1.rename({'n_det_class': 'det_class', 'n_ID': 'det_ID', 'nearest_point': 'geometry'}, axis=1)
        FP1_geometry = FP1.geometry
        FP1 = gpd.GeoDataFrame(FP1, geometry=FP1_geometry, crs=IRENET95)
